...
- **Deduplication**: Pull route compares against the actual SendGrid list via search/emails API (batched at 100), not just local DB flags
...
## Workflow
1. User selects data source (Salesforce or Mailchimp) and picks a report/audience
2. User clicks "Fetch Data" to pull contacts from the selected source
3. System compares against existing database contacts by email and against the SendGrid list
4. New contacts are saved to the database
5. User selects a SendGrid marketing list and clicks "Upload"
